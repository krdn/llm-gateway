...
// src/gateway/select-strategy.ts
function selectStrategy(provider) {
  const meta = PROVIDER_REGISTRY[provider];
...
async function executeStrategy(strategy, model, schema, opts) {
  switch (strategy) {
    case "native":
...
- If information is missing, use reasonable defaults ("" for strings, 0 for numbers, [] for arrays).`;
  const analysisSnippet = step1.text.substring(0, 2e3);
